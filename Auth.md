...
NexFlux supports OAuth 2.0 authentication with the following providers:
- **Google**
- **GitHub**
- **Apple**
...
|-----------|--------|-------------|
| provider  | string | OAuth provider: `google`, `github`, `apple` |

**Query Parameters (Optional)**

| Parameter | Type   | Description |
|-----------|--------|-------------|
| redirect_uri | string | Custom redirect URI after OAuth |
...
Handles OAuth callback from provider (used when provider redirects back).
...
```
{FRONTEND_URL}/auth/{provider}/callback?token=jwt-token-here
```

Or redirects with error:
```
{FRONTEND_URL}/auth/{provider}/callback?error=access_denied&error_description=...
```

---

#### 4. OAuth Callback (POST)

**POST** `/auth/{provider}/callback`

Handles OAuth callback via POST (for frontend-to-backend code exchange).
...
```json
{
  ...
  "redirect_uri": "https://app.nexflux.io/auth/google/callback"
}
```
...
4. Check if user exists (by email or provider_user_id)
5. If new user:
   - Create user record
   - Create oauth_accounts entry
   - Generate JWT token
6. If existing user:
   - Update or create oauth_accounts entry
   - Generate JWT token
...
# OAuth Callback URL
OAUTH_REDIRECT_BASE_URL=https://app.nexflux.io/auth
```
...
- [ ] GET `/auth/google` - Get Google authorization URL
- [ ] GET `/auth/github` - Get GitHub authorization URL
- [ ] GET `/auth/apple` - Get Apple authorization URL
- [ ] GET `/auth/{provider}/callback` - Handle OAuth callback (redirect)
- [ ] POST `/auth/{provider}/callback` - Handle OAuth callback (API)
